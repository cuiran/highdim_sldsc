...
if __name__ == '__main__':
    ...
    for i in range(num_simulations):
        ...
        original_data = d.data('X.h5','y.txt','w.txt',[[0,1000]])
    ...
    df.to_csv('testprocess_samealpha_alpha_wsse.csv',sep='\t',index=False)
    coef_df = pd.DataFrame(data=klasso_coef,columns=[x for x in range(1,len(klasso_coef[0])+1)])
    coef_df.to_csv('testprocess_samealpha_klassocoef_sim.csv',sep='\t',index=False)
    coef_df = pd.DataFrame(data=sklasso_coef,columns=[x for x in range(1,len(sklasso_coef[0])+1)])
    coef_df.to_csv('testprocess_samealpha_sklassocoef_sim.csv',sep='\t',index=False)
